...
public class Weapon : MonoBehaviour
{
    ...
    {
        if (Ammo.clipAmmo <= 0 || resettingCartridge || !weaponSettings.bulletSpan)
            return;
        ...
        if (Physics.Raycast(bSpawnPoint, dir, out hit, weaponSettings.range, weaponSettings.bulletLayers))
        {
            ...
            {
                Debug.Log("hm yahan hein");
                hit.transform.GetComponent<Stats>().Damage();  
            }
    ...
    {
        pv.RPC("RPC_Sounds", RpcTarget.All);
    }
    ...
    public void Equip()
    {
        pv.RPC("RPC_Equip", RpcTarget.All);
    ...
    public void Unequip()
    {
        pv.RPC("RPC_Unequip", RpcTarget.All);
    ...
    void RPC_Unequip(WeaponType wpType)
    {
        if (!owner)
            return;
