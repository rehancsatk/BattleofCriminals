...
{
    public MyPlayer characterMove { get; protected set; }
    ...
    public class InputSettings
    {
        ...
        //public string Pickweaponbutton = "PickWeapon";
    ...
    public class OtherSettings
    {
        ...
        public bool reqireInputForTurn { get; set; }
    }
     [SerializeField]
    ...
    void Start()
    {
        ...
        characterMove = GetComponent<MyPlayer>();
    ...
    {
        if (weaponHandler.OrginalList.Count > 0)
        {
            foreach (Weapon wep in weaponHandler.OrginalList)
            {
                GameObject prefab = wep.weaponSettings.CrossHiar_Prefab;
                if (prefab != null)
                {
   ...
   void CameraLookLogic()
   {
     ...
     others.requireInputForTurn = !aiming; // es line ko na hatae spine hi khali move kre ghi bhae phr

        if (others.requireInputForTurn)
        {
   ...
   void CharacterLook()
    {
        ...
        Vector3 lookTarget = pivotPos + (pivotT.forward * others.lookDistance);
       ...
       // by default character ka camera yeh hu ga
        Quaternion newRotation = Quaternion.Lerp(transform.rotation, lookRot, Time.deltaTime * others.lookSpeed);
    ...
    void WeaponLogic()
    {
        ...
        aiming = Input.GetButton(input.aimbutton) || debugAim;
          ...
          // others.reqireInputForTurn = !aiming;
           ...
           // if (Input.GetButtonDown(input.dropWeaponButton))
            ...
            if (weaponHandler.currentWeapon)
            {
                ...
                {
                    weaponHandler.RPC_FireCurrentWeapon(aimRay); 
                }
			if (Input.GetButtonDown (input.reloadbutton))
				weaponHandler.Reload ();
			if (Input.GetButtonDown (input.dropWeaponButton)) {
    ...
    void PositionCrosshair(Ray ray, Weapon wep)
    {
        ...
        RaycastHit hit;
        Transform bSpawn = curWeapon.weaponSettings.bulletSpan;
    ...
    void UpdateCrosshairs()
    {
        ...
        foreach (Weapon wep in weaponHandler.weaponList)
        {
